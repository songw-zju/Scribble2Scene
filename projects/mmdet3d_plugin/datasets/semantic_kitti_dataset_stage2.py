...
from projects.mmdet3d_plugin.scribble2scene.utils.ssc_metric import SSCMetrics
...
class SemanticKittiDatasetStage2(Dataset):
    ...
    def get_data_info(self, index):
        ...
        scribble, pseudo = self.get_scribble_info(sequence, frame_id)
    ...
    def get_gt_info(self, sequence, frame_id):
        ...
        if self.split == "train" or self.split == "val":
            ...
            target_path = os.path.join("./data/semantic_kitti/dataset/sequences", sequence,
    ...
    def get_scribble_info(self, sequence, frame_id):
        if self.split == "train" or self.split == "val":
            # load full-range groundtruth
            scribble_path = os.path.join("./data/scribble_kitti/sequences", sequence,
                                         "voxels", frame_id + ".label")
            pseudo_path = os.path.join("./data/out_scpnet/val/sequences", sequence,
                                         "predictions", frame_id + ".label")
            target_path = os.path.join("./data/semantic_kitti/dataset/sequences", sequence,
                                       "voxels", frame_id + ".label")

            scribble_annotated_data = np.fromfile(scribble_path, dtype=np.uint16).reshape((-1, 1))
            pseudo_label = np.fromfile(pseudo_path, dtype=np.uint16).reshape((-1, 1))

            scribble_annotated_data = self.comletion_remap_lut[scribble_annotated_data]

            pseudo_label = self.comletion_remap_lut[pseudo_label]
            pseudo_label[scribble_annotated_data != 0] = scribble_annotated_data[scribble_annotated_data != 0]
            # scribble_annotated_data[scribble_annotated_data == 0] = \
            #     pseudo_label[scribble_annotated_data == 0]

            invalid_name = target_path.replace('.label', '.invalid')
            invalid_voxels = unpack(np.fromfile(invalid_name, dtype=np.uint8))

            pseudo_label[np.isclose(invalid_voxels, 1)] = 255
            # scribble_annotated_data[np.isclose(invalid_voxels, 1)] = 255

            scribble_annotated_data = scribble_annotated_data.reshape((256, 256, 32))
            pseudo_label = pseudo_label.reshape((256, 256, 32))

        else:
            scribble_annotated_data = np.ones((256, 256, 32))
            pseudo_label = np.ones((256, 256, 32))

        return scribble_annotated_data.astype(np.float32), pseudo_label.astype(np.float32)
