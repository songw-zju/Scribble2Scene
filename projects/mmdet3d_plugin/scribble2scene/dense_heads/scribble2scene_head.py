...
from projects.mmdet3d_plugin.scribble2scene.utils.header import Header
from projects.mmdet3d_plugin.scribble2scene.utils.ssc_loss import sem_scal_loss, geo_scal_loss, CE_ssc_loss
from projects.mmdet3d_plugin.scribble2scene.utils.distillation_loss import range_guided_distill_logit_loss, distill_feat
from projects.mmdet3d_plugin.models.utils.bricks import run_time


@HEADS.register_module()
class Scribble2SceneHead(nn.Module):
    def __init__(
        self,
        *args,
        bev_h,
    ...
    def forward(self, mlvl_feats, img_metas, target, scribble, pseudo, use_teacher=False):
        """Forward function.
        Args:
        ...
        if use_teacher:
            proposal = img_metas[0]['gt_proposal'].reshape(self.bev_h, self.bev_w, self.bev_z)
        else:
            proposal = img_metas[0]['proposal'].reshape(self.bev_h, self.bev_w, self.bev_z)
            # geo_output = self.comp_subnetwork(proposal)
        ...
        # vox_feats_flatten_distill = copy.deepcopy(vox_feats_flatten)
    ...
    def step(self, out_dict, teacher_out_dict, stu_feat, tea_feat, target, scribble, pseudo, img, img_metas, step_type):
        ...
        if step_type== "train":
            ...
            if self.CE_ssc_loss:
                # loss_ssc = CE_ssc_loss(ssc_pred, scribble, class_weight)
                loss_ssc = CE_ssc_loss(ssc_pred, pseudo, class_weight)
                loss_dict['loss_ssc'] = loss_ssc

            if self.sem_scal_loss:
                loss_sem_scal = sem_scal_loss(ssc_pred, pseudo)
                loss_dict['loss_sem_scal'] = loss_sem_scal

            if self.geo_scal_loss:
                loss_geo_scal = geo_scal_loss(ssc_pred, pseudo)
                loss_dict['loss_geo_scal'] = loss_geo_scal

            distill_mask = ((target != 0) & (target != 255))
            if use_distill:
                teacher_ssc_pred = teacher_out_dict["ssc_logit"]
                abl_eff = 1.0

                # logit-level distillation loss
                loss_logit_distill_short, loss_logit_distill_middle, loss_logit_distill_long = \
                    range_guided_distill_logit_loss(ssc_pred, teacher_ssc_pred, scribble, distill_mask)
                loss_dict['loss_distill_short'] = loss_logit_distill_short * 0.06 * abl_eff
                loss_dict['loss_distill_middle'] = loss_logit_distill_middle * 0.15 * abl_eff
                loss_dict['loss_distill_long'] = loss_logit_distill_long * 0.20 * abl_eff

                # feature-level distillation loss
                loss_feat_distill = distill_feat(stu_feat['image_feats'], tea_feat['image_feats'])
                loss_dict['loss_feat_distill'] = loss_feat_distill * abl_eff
    ...
    def training_step(self, out_dict, teacher_out_dict, stu_feat, tea_feat, target, scribble, pseudo, img, img_metas):
        """Training step.
        """
        return self.step(out_dict, teacher_out_dict, stu_feat, tea_feat, target, scribble, pseudo, img, img_metas, "train")

    def validation_step(self, out_dict, target, scribble, pseudo, img_metas):
        """Validation step.
        """
        return self.step(out_dict, None, None, None, target, scribble, pseudo, None, img_metas, "val")
    ...
    def save_pred(self, img_metas, y_pred):
        ...
        # save predictions
        pred_folder = os.path.join("./scribble2scene", "sequences", img_metas[0]['sequence_id'], "predictions")
